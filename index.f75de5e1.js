...
function camelCaseToReadable(str) {
    return str.replace(/([a-z])([A-Z])/g, "$1 $2").replace(/^./, function(match) {
...
function addLabels(forms) {
    forms.forEach((form)=>{
        ...
        inputs.forEach((formInput)=>{
            formInput.insertAdjacentHTML("beforebegin", '<label class="field-label" for="' + formInput.id + '">' + camelCaseToReadable(formInput.name) + "</label>");
